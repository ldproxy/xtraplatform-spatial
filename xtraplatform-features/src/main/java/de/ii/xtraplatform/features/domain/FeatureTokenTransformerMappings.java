...
public class FeatureTokenTransformerMappings extends FeatureTokenTransformer {
  ...
  public void onFeatureStart(ModifiableContext<FeatureSchema, SchemaMapping> context) {
    ...
    // Propagate per-feature state set by upstream transformers (e.g. role-as-link captures
    // from FeatureTokenTransformerLinkRoles, canonical id captured by
    // FeatureTokenTransformerCompositeId) into the rebuilt context that downstream sees.
    newContext.setRoleLinks(context.roleLinks());
  ...
  public void onFeatureEnd(ModifiableContext<FeatureSchema, SchemaMapping> context) {
    ...
    // Upstream transformers (LinkRoles, CompositeId) may have populated per-feature context
