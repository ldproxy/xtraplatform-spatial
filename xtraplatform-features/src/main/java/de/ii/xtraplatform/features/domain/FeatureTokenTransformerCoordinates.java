...
public class FeatureTokenTransformerCoordinates extends FeatureTokenTransformer {
  ...
  public void onGeometry(ModifiableContext<FeatureSchema, SchemaMapping> context) {
    ...
    if (propertyCrs.isPresent()) {
      transformToOriginalCrs(context, geometry, propertyCrs.get());
      getDownstream().onGeometry(context);
      return;
    }

    transformToTargetCrs(context, geometry);

    getDownstream().onGeometry(context);
  }

  private void transformToOriginalCrs(
    ...
    if (originalCrs.isEmpty() || originalCrs.get().equals(propertyCrs)) {
      return;
    }

    Optional<CrsTransformer> toOriginal =
        crsTransformerFactory.getTransformer(propertyCrs, originalCrs.get());
  ...
  private void transformToTargetCrs(
      ModifiableContext<FeatureSchema, SchemaMapping> context, Geometry<?> geometry) {
    ...
    if (next != null) {
      context.setGeometry(geometry.accept(new CoordinatesTransformer(next)));
    }
  }
}
