...
public interface Tile3dGenerationOptions {
  ...
  /**
   * @langEn `min` is the first level of the tileset which will contain buildings. The value will
   *     depend on the spatial extent of the dataset, i.e., at what level of the implicit tiling
   *     scheme large buildings can be displayed.<br>
   *     `max` is the last level of the tileset which will contain buildings. The value will depend
   *     on the spatial extent of the dataset, i.e., at what level of the implicit tiling scheme
   *     small buildings can be displayed in detail.
   * @langDe `min` ist die erste Ebene des Kachelsatzes, die Gebäude enthalten wird. Der Wert hängt
   *     von der räumlichen Ausdehnung des Datensatzes ab, d. h. davon, auf welcher Ebene des
   *     impliziten Kachelschemas große Gebäude dargestellt werden können. `max` ist die letzte
   *     Ebene des Kachelsatzes, die Gebäude enthalten wird. Der Wert hängt von der räumlichen
   *     Ausdehnung des Datensatzes ab, d. h. davon, auf welcher Ebene des impliziten Kachelschemas
  ...
  @Value.Derived
  @Value.Auxiliary
  default Map<String, MinMax> getLevels() {
    return Objects.nonNull(getContentLevels()) ? Map.of("default", getContentLevels()) : Map.of();
  }
